...
export default function AdminEducationPage() {
  ...
  async function loadEducation() {
    try {
      ...
      if (!Array.isArray(data)) {
        throw new Error("Invalid education data received.");
      }
  ...
  async function deleteEducation(item: Education) {
    const confirmed = window.confirm(
      `Delete "${item.degree}" at "${item.institution}"?\n\nThis action cannot be undone.`,
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail || "Failed to delete education.",
        );
      }
  ...
  function formatYears(item: Education) {
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-[#777]">
                Manage your academic background and control
